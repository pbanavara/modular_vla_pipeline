#!/usr/bin/env python3
"""
Llama-optimized LLM Planner Example
Demonstrates high-performance LLM planning with Llama models via Lambda Labs API.
"""
...
def test_llama_connection():
    """Test Llama API connection via Lambda Labs."""
    print("=== Llama API Connection Test ===")
...
def benchmark_single_requests():
    """Benchmark single request performance with Llama."""
    print("\n=== Single Request Performance Benchmark (Llama) ===")
    
    if not validate_config("lambda_labs"):
        print("❌ Lambda Labs API key not found.")
        return None
    
    # Initialize Llama-optimized planner
    ...
    print("🔥 Testing single request performance with Llama...")
...
def benchmark_batch_requests():
    """Benchmark batch request performance with Llama."""
    print("\n=== Batch Request Performance Benchmark (Llama) ===")
    ...
    print(f"🔥 Testing batch processing of {len(tasks)} tasks with Llama...")
...
async def benchmark_async_requests():
    """Benchmark async request performance with Llama."""
    print("\n=== Async Request Performance Benchmark (Llama) ===")
    ...
    print("🔥 Testing async processing with Llama...")
    start_time = time.time()
...
def benchmark_cache_performance():
    ...
    print("\n=== Cache Performance Benchmark (Llama) ===")
...
def benchmark_different_llama_models():
    """Benchmark different Llama model configurations."""
    print("\n=== Llama Model Configuration Benchmark ===")
    ...
    models = [
        {"name": "Llama 3.2 70B", "model": "llama-3.2-70b-instruct"},
        {"name": "Llama 3.2 8B", "model": "llama-3.2-8b-instruct"},
        {"name": "Llama 3.1 70B", "model": "llama-3.1-70b-instruct"},
    ]
    ...
    for model_config in models:
        ...
        # Benchmark
        start_time = time.time()
        for _ in range(2):  # Reduced for faster testing
            planner.build_action_plan(task, perception_output, positions)
...
def demonstrate_llama_specific_features():
    """Demonstrate Llama-specific features and optimizations."""
    print("\n=== Llama-Specific Features Demonstration ===")
    ...
    # Test with structured task
    task = "pick up the red cup and place it on the table"
    perception_output = [
        {"name": "red_cup", "labels": ["cup", "red", "container"]},
        {"name": "table_surface", "labels": ["table", "surface", "flat"]}
    ]
    positions = {
        "red_cup": [0.3, -0.2, -0.35],
        "table_surface": [0.4, 0.1, -0.45]
    }
    
    print("🔥 Testing Llama-optimized prompt generation...")
    start_time = time.time()
    
    plan = planner.build_action_plan(task, perception_output, positions)
    
    elapsed_time = time.time() - start_time
    print(f"⏱️  Llama plan generation: {elapsed_time:.3f}s")
    ...
    try:
        ...
        for i, step in enumerate(plan_data[:3]):  # Show first 3 steps
            print(f"   Step {i+1}: {step.get('action', 'unknown')} - {step.get('description', 'no description')}")
        
        if len(plan_data) > 3:
            print(f"   ... and {len(plan_data) - 3} more steps")
            
    except json.JSONDecodeError:
        print("⚠️  Could not parse plan as JSON")
        print(f"Raw response: {plan[:200]}...")
    
    return planner

def main():
    """Run all Llama planner performance benchmarks."""
    print("🚀 Llama-Optimized LLM Planner Performance Benchmarks")
    print("=" * 70)
    
    # Check if robot YAML file exists
    yaml_path = "src/planning/llama.yaml"
    if not os.path.exists(yaml_path):
        print(f"⚠️  Warning: Llama YAML file not found at {yaml_path}")
        print("Please ensure the llama.yaml file exists for full functionality.")
    ...
    try:
        ...
        print("\n" + "=" * 70)
        benchmark_different_llama_models()
        ...
        print("\n" + "=" * 70)
        print("🏁 LLAMA PLANNER PERFORMANCE SUMMARY")
        print("=" * 70)
        ...
        print("   ✅ Structured, step-by-step instructions")
        print("   ✅ Explicit context and role definitions")
        print("   ✅ Clear action vocabulary")
        print("   ✅ Safety constraints and workspace limits")
        ...
        print("\n💡 Llama Usage Tips:")
        print("   • Use structured prompts with clear instructions")
        ...
        print("   • Llama models work well with explicit examples")
        print("   • Use step-by-step instructions for complex tasks")
